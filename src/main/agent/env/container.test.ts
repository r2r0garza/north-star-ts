...
for (const runtime of ["docker", "podman"] as const) {
  describe.skipIf(!available(runtime))(`ContainerEnvironment (${runtime})`, () => {
    ...
    it("isolates writes outside the mount from the host", async () => {
      ...
      expect(hostEntries.find((e) => e.name === "inside-only")).toBeUndefined()
    })
    ...
    it("search finds a line via a single in-container command", async () => {
      await env.writeFile(await env.resolve("hay.txt"), "alpha\n84f2348602b2\ngamma")
