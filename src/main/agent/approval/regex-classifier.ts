...
const DANGEROUS_PATTERNS: Pattern[] = [
  ...
  [String.raw`\brm\s+--recursive\b`, "recursive delete (long flag)", "destructive_fs"],
  [String.raw`\bchmod\s+(-[^\s]*\s+)*(777|666|o\+[rwx]*w|a\+[rwx]*w)\b`, "world/other-writable permissions", "system_mutation"],
  [String.raw`\bchmod\s+--recursive\b.*(777|666|o\+[rwx]*w|a\+[rwx]*w)`, "recursive world/other-writable (long flag)", "system_mutation"],
  [String.raw`\bchown\s+(-[^\s]*)?r\s+root`, "recursive chown to root", "system_mutation"],
  [String.raw`\bchown\s+--recursive\b.*root`, "recursive chown to root (long flag)", "system_mutation"],
  ...
  [String.raw`\bdelete\s+from\b(?![^\n]*\bwhere\b)`, "SQL DELETE without WHERE", "destructive_fs"],
  [String.raw`\btruncate\s+(table)?\s*\w`, "SQL TRUNCATE", "destructive_fs"],
  [String.raw`>\s*${SYSTEM_CONFIG_PATH}`, "overwrite system config", "system_mutation"],
  [String.raw`\bsystemctl\s+(-[^\s]+\s+)*(stop|restart|disable|mask)\b`, "stop/restart system service", "system_mutation"],
  ...
  [String.raw`\bkillall\s+(-[^\s]*\s+)*-(9|kill|sigkill)\b`, "force kill processes (killall -KILL)", "system_mutation"],
  [String.raw`\bkillall\s+(-[^\s]*\s+)*-s\s+(kill|sigkill|9)\b`, "force kill processes (killall -s KILL)", "system_mutation"],
  [String.raw`\bkillall\s+(-[^\s]*\s+)*-r\b`, "kill processes by regex (killall -r)", "system_mutation"],
  [String.raw`:\(\)\s*\{\s*:\s*\|\s*:\s*&\s*\}\s*;\s*:`, "fork bomb", "system_mutation"],
  [String.raw`\b(bash|sh|zsh|ksh)\s+-[^\s]*c(\s+|$)`, "shell command via -c/-lc flag", "code_exec"],
  [String.raw`\b(python[23]?|perl|ruby|node)\s+-[ec]\s+`, "script execution via -e/-c flag", "code_exec"],
  [String.raw`\b(curl|wget)\b.*\|\s*(?:[/\w]*/)?(?:ba)?sh(?:\s|$|-c)`, "pipe remote content to shell", "code_exec"],
  [String.raw`\b(bash|sh|zsh|ksh)\s+<\s*<?\s*\(\s*(curl|wget)\b`, "execute remote script via process substitution", "code_exec"],
  [String.raw`\btee\b.*["']?${SENSITIVE_WRITE_TARGET}`, "overwrite system file via tee", "system_mutation"],
  [String.raw`>>?\s*["']?${SENSITIVE_WRITE_TARGET}`, "overwrite system file via redirection", "system_mutation"],
  [String.raw`\btee\b.*["']?${PROJECT_SENSITIVE_WRITE_TARGET}["']?${COMMAND_TAIL}`, "overwrite project env/config via tee", "system_mutation"],
  [String.raw`>>?\s*["']?${PROJECT_SENSITIVE_WRITE_TARGET}["']?${COMMAND_TAIL}`, "overwrite project env/config via redirection", "system_mutation"],
  [String.raw`\bxargs\s+.*\brm\b`, "xargs with rm", "destructive_fs"],
  [String.raw`\bfind\b.*-exec(?:dir)?\s+(/\S*/)?rm\b`, "find -exec/-execdir rm", "destructive_fs"],
  [String.raw`\bfind\b.*-delete\b`, "find -delete", "destructive_fs"],
  [String.raw`\b(cp|mv|install)\b.*\s${SYSTEM_CONFIG_PATH}`, "copy/move file into system config path", "system_mutation"],
  [String.raw`\b(cp|mv|install)\b.*\s["']?${PROJECT_SENSITIVE_WRITE_TARGET}["']?${COMMAND_TAIL}`, "overwrite project env/config file", "system_mutation"],
  [
  ...
  [String.raw`\bsed\s+-[^\s]*i.*(?:${USER_SENSITIVE_WRITE_TARGET})[^\s"']*`, "in-place edit of sensitive credential/SSH/shell-rc path", "system_mutation"],
  [String.raw`\bsed\s+--in-place\b.*(?:${USER_SENSITIVE_WRITE_TARGET})[^\s"']*`, "in-place edit of sensitive credential/SSH/shell-rc path (long flag)", "system_mutation"],
  [String.raw`\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:${USER_SENSITIVE_WRITE_TARGET})[^\s"']*`, "in-place edit of sensitive credential/SSH/shell-rc path (perl/ruby)", "system_mutation"],
  [String.raw`\bsed\s+-[^\s]*i.*\s${SYSTEM_CONFIG_PATH}`, "in-place edit of system config", "system_mutation"],
  [String.raw`\bsed\s+--in-place\b.*\s${SYSTEM_CONFIG_PATH}`, "in-place edit of system config (long flag)", "system_mutation"],
  [String.raw`\b(python[23]?|perl|ruby|node)\s+<<`, "script execution via heredoc", "code_exec"],
  [String.raw`\bgit\s+reset\s+--hard\b`, "git reset --hard (destroys uncommitted changes)", "history_rewrite"],
  [String.raw`\bgit\s+push\b.*--force\b`, "git force push (rewrites remote history)", "history_rewrite"],
  [String.raw`\bgit\s+push\b.*-f\b`, "git force push short flag (rewrites remote history)", "history_rewrite"],
  [String.raw`\bgit\s+clean\s+-[^\s]*f`, "git clean with force (deletes untracked files)", "destructive_fs"],
  [String.raw`\bgit\s+branch\s+-d\b`, "git branch force delete", "history_rewrite"],
  [String.raw`\bchmod\s+\+x\b.*[;&|]+\s*\./`, "chmod +x followed by immediate execution", "code_exec"],
  [String.raw`\bsudo\b[^;|&\n]*?\s+(?:-s\b|--stdin\b|-a\b|--askpass\b)`, "sudo with privilege flag (stdin/askpass/shell/list)", "system_mutation"],
  [String.raw`\bsudo\b[^;|&\n]*?\s+-[a-z]*[sa][a-z]*\b`, "sudo with combined-flag privilege escalation", "system_mutation"],
]
