// Single source of truth for the app's customizable brand theme. Two optional
// env vars (read from .env.local, loaded by dotenv in index.ts) rebrand the UI
// colors without touching code:
//
//   NEXT_accent_color   — the brand/primary hue (a hex like "#008236").
//   NEXT_neutral_color  — the surface/background base (a hex like "#0a0a0a").
//
// Both DEFAULT to the app's current green + near-black neutral, so an unset (or
// default-valued) config produces the exact colors shipped in globals.css.
//
// Approach (see the design discussion): we keep each design token's carefully
// tuned LIGHTNESS ladder from globals.css and only re-color it —
...
// is ~pure gray, passing the current hexes reproduces today's UI (any drift is
// sub-perceptual chroma rounding). `destructive` and the chart colors are
// semantic/fixed and are intentionally NOT recolored.
//
...
const DEFAULT_ACCENT_HEX = "#008236"
const DEFAULT_NEUTRAL_HEX = "#0a0a0a"
...
// The customizable brand theme. Returns the CSS custom-property declarations for
// `:root` (light) and `.dark`, ready to inject as an override <style> after
// globals.css. Returns null when neither env var is set AND the seeds resolve to
// the defaults, i.e. nothing to override (the renderer then leaves globals.css
// untouched). Invalid hex values fall back to the defaults.
export function brandThemeCss(): { light: string; dark: string } | null {
  ...
  const accent =
    (accentRaw && hexToOklch(accentRaw)) || hexToOklch(DEFAULT_ACCENT_HEX)!
  const neutral =
    (neutralRaw && hexToOklch(neutralRaw)) || hexToOklch(DEFAULT_NEUTRAL_HEX)!
