...
1. **`094` — Context history tail query.** Push the rolling-summary `seq` boundary into SQLite so
   context construction does not load, map, and JSON-parse messages that the summary already replaces.
   Preserve intentional full-history replay when no summary exists; re-check boundary semantics and the
   existing composite-index query plan in the required pre-implementation analysis pass.
2. **`095` — Lightweight Markdown rendering while streaming.** Give live assistant Markdown an explicit
   lightweight mode that skips repeated whole-response syntax highlighting and incomplete Mermaid rendering,
   then restores the full renderer when the turn settles. Analyze segment/retry behavior and profile a
   representative code-heavy stream before locking the implementation; coordinate with `093`'s transcript edits.
3. **`096` — Leak-free command-session settlement waits.** Replace per-wait `onExit` subscriptions with
   session-owned one-shot settlement promises in both command and diagnostic sessions. Re-audit timeout,
   interrupt, kill, abort, and cleanup transitions first, then add listener-count and timer-race regressions.
4. **`093` — Conversation message copy actions and timestamps.** Add a compact metadata/action row
...
7. **`082` — Runtime-aware Process run-monitor badges.** Render each phase's actual persisted runtime
...
8. **`083` — Per-agent runtime override UI.** Expose the existing
...
10. **`069` — Process intake policies and inspectable assumptions.** Give each Process an explicit run-
...
11. **`039` — Inspectable Process consultations / Agent exchanges.** A running phase may consult a
