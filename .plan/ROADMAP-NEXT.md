...
1. **`106.3` — Mission Control: playbooks and slice execution (MVP).** Playbooks at slice/mission/
...
2. **`106.4` — Mission Control: seat sessions and Comms.** Long-lived per-seat sessions with
...
3. **`106.5` — Mission Control: worktrees, merge queue, mission merge policy.** Worktree per slice off
...
4. **`106.6` — Mission Control: Navigator and autonomous drive.** Deterministic, restart-safe Navigator
    ...
    with auto-pause.
5. **`106.7` — Mission Control: Refocus, follow-ups, seat memory.** Intent-chain reminders at kickoff,
...
6. **`106.8` — Mission Control: health monitoring.** Progress vs ceremony event classification,
...
7. **`106.9` — Mission Control: Processes sunset.** Parity checklist, import Processes as playbooks,
...
8. **`105` — Required skills for individual Process phases.** Let an agent phase explicitly activate one
...
9. **`092.1` — Agent lifecycle hook contract, storage, runner, and safety controls.** Define versioned
...
10. **`045` — North Star MCP bridge for CLI providers.** After `042`, make North Star a second, distinct
