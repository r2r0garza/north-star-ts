...
1. **`106.1` — Mission Control: rig topology (seats, pods, rigs).** First slice of the `106` umbrella
   (Processes revamp). Durable Seat → Pod → Rig model with stable `seat@pod` addresses, charters,
   decision rights, per-seat runtime, pod oversight DAG, rig/pod culture, portable rig templates, and the
   Mission Control sidebar shell + Rigs tab. Built alongside Processes; no execution yet.
2. **`106.2` — Mission Control: initiatives, missions, slices, waves.** User-authored work map with
   structured slice specs, same-mission slice DAGs rendered as derived waves, rig snapshot at start,
   audit log of structural revisions, and a zoomable initiative → mission → slice UI.
3. **`106.3` — Mission Control: playbooks and slice execution (MVP).** Playbooks at slice/mission/
    initiative altitude compiled onto the existing Process engine with seat-role binding; folds in the
    former `039.1` explicit phase-result integrity for all Processes; structured, independently verified,
    freeze-on-accept slice proofs. Evaluate seat-bound execution here before investing in autonomy.
4. **`106.4` — Mission Control: seat sessions and Comms.** Long-lived per-seat sessions with
...
5. **`106.5` — Mission Control: worktrees, merge queue, mission merge policy.** Worktree per slice off
...
6. **`106.6` — Mission Control: Navigator and autonomous drive.** Deterministic, restart-safe Navigator
    ...
    with auto-pause.
7. **`106.7` — Mission Control: Refocus, follow-ups, seat memory.** Intent-chain reminders at kickoff,
...
8. **`106.8` — Mission Control: health monitoring.** Progress vs ceremony event classification,
...
9. **`106.9` — Mission Control: Processes sunset.** Parity checklist, import Processes as playbooks,
...
10. **`105` — Required skills for individual Process phases.** Let an agent phase explicitly activate one
...
11. **`092.1` — Agent lifecycle hook contract, storage, runner, and safety controls.** Define versioned
...
12. **`045` — North Star MCP bridge for CLI providers.** After `042`, make North Star a second, distinct
...
13. **`067` — Conversation-scoped workspace checkpoints.** Add a reversible safety layer for autonomous
...
14. **`069` — Process intake policies and inspectable assumptions.** Give each Process an explicit run-
...
15. **`084` — Process import account-remapping UX.** Analyze imported Process runtime selections before
...
16. **`085` — Process template library.** Add a small, polished built-in catalog of portable starter
