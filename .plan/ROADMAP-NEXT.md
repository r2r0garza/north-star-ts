...
1. **`097` — Transcript render boundaries and measured virtualization.** With `095` and the overlapping
...
2. **`033.4` — Agent-readable live dashboards. DISCUSSION REQUIRED BEFORE IMPLEMENTATION.** Add a
...
3. **`104` — Deterministic command phases.** Let a Process phase execute a persisted, user-reviewed
...
4. **`105` — Required skills for individual Process phases.** Let an agent phase explicitly activate one
