...
1. **`103` — Search conversations by title and transcript content.** Add a magnifying-glass button
   between the active view's new-conversation and new-project actions, open an accessible search dialog,
   and rank user-facing conversations by title and persisted visible transcript matches using bounded
   SQLite full-text search. Reuse the existing mode-aware conversation-selection path when a result is
   chosen, and keep private task-worker transcripts and internal tool content out of results.
2. **`102` — Atomic summary-boundary admission.** Fix the confirmed context-integrity gap where a
   ...
   admitted.
3. **`099` — Hardened host attachment reads.** In a focused filesystem-hardening pass, distinguish user
...
4. **`097` — Transcript render boundaries and measured virtualization.** With `095` and the overlapping
...
5. **`033.4` — Agent-readable live dashboards. DISCUSSION REQUIRED BEFORE IMPLEMENTATION.** Add a
...
6. **`092.1` — Agent lifecycle hook contract, storage, runner, and safety controls.** Define versioned
...
7. **`045` — North Star MCP bridge for CLI providers.** After `042`, make North Star a second, distinct
...
8. **`082` — Runtime-aware Process run-monitor badges.** Render each phase's actual persisted runtime
...
9. **`083` — Per-agent runtime override UI.** Expose the existing
...
12. **`039` — Inspectable Process consultations / Agent exchanges.** A running phase may consult a
...
13. **`084` — Process import account-remapping UX.** Analyze imported Process runtime selections before
...
14. **`085` — Process template library.** Add a small, polished built-in catalog of portable starter
