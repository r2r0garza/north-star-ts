...
1. **`099` — Hardened host attachment reads.** In a focused filesystem-hardening pass, distinguish user
...
2. **`097` — Transcript render boundaries and measured virtualization.** With `095` and the overlapping
...
3. **`033.4` — Agent-readable live dashboards. DISCUSSION REQUIRED BEFORE IMPLEMENTATION.** Add a
...
4. **`104` — Deterministic command phases.** Let a Process phase execute a persisted, user-reviewed
