...
1. **`095` — Lightweight Markdown rendering while streaming.** Give live assistant Markdown an explicit
...
2. **`096` — Leak-free command-session settlement waits.** Replace per-wait `onExit` subscriptions with
...
3. **`093` — Conversation message copy actions and timestamps.** Add a compact metadata/action row
...
6. **`082` — Runtime-aware Process run-monitor badges.** Render each phase's actual persisted runtime
...
7. **`083` — Per-agent runtime override UI.** Expose the existing
...
8. **`067` — Conversation-scoped workspace checkpoints.** Add a reversible safety layer for autonomous
...
9. **`069` — Process intake policies and inspectable assumptions.** Give each Process an explicit run-
...
10. **`039` — Inspectable Process consultations / Agent exchanges.** A running phase may consult a
...
11. **`084` — Process import account-remapping UX.** Analyze imported Process runtime selections before
...
12. **`085` — Process template library.** Add a small, polished built-in catalog of portable starter
