...
1. **`096` — Leak-free command-session settlement waits.** Replace per-wait `onExit` subscriptions with
...
2. **`093` — Conversation message copy actions and timestamps.** Add a compact metadata/action row
...
8. **`069` — Process intake policies and inspectable assumptions.** Give each Process an explicit run-
...
9. **`039` — Inspectable Process consultations / Agent exchanges.** A running phase may consult a
...
10. **`084` — Process import account-remapping UX.** Analyze imported Process runtime selections before
...
11. **`085` — Process template library.** Add a small, polished built-in catalog of portable starter
